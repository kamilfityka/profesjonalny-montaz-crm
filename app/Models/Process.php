 ...
 */
class Process extends PraustActionModel
{
	use PraustCategory;
    ...
    public function fields(bool $construct = false): array
    {
        ...
        $info = [];
        if (!$construct) {
            foreach ((new ProcessType())->newQuery()->get() as $model) {
        ...
        $arr[] = TextName::make("name")->validate("required")->label('Temat');
        $arr[] = Select::make("type_id")->label('Typ')->options($info)->addSelectText();
        $arr[] = DateTime::make("closed_at")->label('Data podpisania umowy');
        $arr[] = TextInput::make("value")->label('Wartość');
